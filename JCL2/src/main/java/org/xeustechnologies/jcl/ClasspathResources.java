                ...
                if( jarEntryContents.containsKey( entryName ) ) {
                    if( !collisionAllowed )
                        ...
                        if( logger.isLoggable( Level.FINER ) )
                            logger.finer( "Resource " + entryName + " already loaded; ignoring entry..." );
                        return;
                    }
                }
        ...
        try {
            ...
            if( jarEntryContents.containsKey( url.toString() ) ) {
                if( !collisionAllowed )
                    ...
                    if( logger.isLoggable( Level.FINER ) )
                        logger.finer( "Resource " + url.toString() + " already loaded; ignoring entry..." );
                    return;
                }
            }
            ...
            if( fis.read( content ) != -1 ) {
                ...
                if( jarEntryContents.containsKey( entryName ) ) {
                    if( !collisionAllowed )
                        ...
                        if( logger.isLoggable( Level.FINER ) )
                            logger.finer( "Class " + entryName + " already loaded; ignoring entry..." );
                        return;
                    }
                }
